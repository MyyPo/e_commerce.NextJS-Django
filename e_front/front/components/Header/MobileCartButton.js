...
function MobileCartButton({
  ...
  cart,
  mobileCartDropdown,
  setMobileCartDropdown,
}) {
  return (
    <button
      onClick={() =>
        mobileCartDropdown
          ? setMobileCartDropdown(false)
          : setMobileCartDropdown(true)
      }
      onBlur={() => setMobileCartDropdown(false)}
    ...
    >
      ...
      <SignInPortal selector="#SignIn">
        <div
          className={`h-full top-1/2 left-0 right-0 z-60 ${
            mobileCartDropdown ? "fixed" : "hidden"
          }`}
        >
